...
gulp.task('vendor', function (cb) {
  ...
  ]).pipe(gulp.dest(PUBLIC_DIR + '/vendor/tinymce'))
...
function jade () {
  ...
  cur = cur.pipe(
    gulpJade({ pretty: IS_PROD ? false : true, locals: { 'te': 2 } }))
...
function static_copy () {
  let cur = gulp.src(SRC_DIR + '/static_copy/**')
  setTimeout(browserSyncReload, 500)
  return cur.pipe(gulp.dest(PUBLIC_DIR + '/'))
}
